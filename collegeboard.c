...
void randomizeDirection(Ball *ball);
void randomizeVelocity(Ball *ball, int lower, int upper);
...
moveBall(Ball *ball, int x, int y)
{
	...
	updateTile((*ball).x, (*ball).y); /* I wrote this function */
	(*ball).x = x;
	(*ball).y = y;
	board[x][y] = BALL;
	updateTile(x, y);
...
play(int level, unsigned int *score, int *lives)
{
	...
	while (*lives > 0) {
		...
		moveBall(&ball, WIDTH / 2, (maxBlockY + paddle.y) / 2);
		randomizeVelocity(&ball, 6, 10);
		randomizeDirection(&ball);
		ball.yDirection = -1;
...
// Has a 50% chance of negating the ball's directions or not.
void
randomizeDirection(Ball *ball)
{
	if (rand() % 2 == 0)
		(*ball).xDirection = -(*ball).xDirection;
	if (rand() % 2 == 0)
...
// Randomizes the ball's velocity in the range [lower, upper].
void
randomizeVelocity(Ball *ball, int lower, int upper)
{
		(*ball).xVelocity = (rand() % upper) + lower;
		(*ball).yVelocity = (rand() % upper) + lower;
}
